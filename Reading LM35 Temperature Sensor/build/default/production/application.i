...
# 12 "./ECU_Layer/LED/../../MCAL_Layer/GPIO/../std_types.h"
...
# 12 "./ECU_Layer/LED/../../MCAL_Layer/GPIO/../std_types.h" 2
...
# 13 "./ECU_Layer/LED/../../MCAL_Layer/GPIO/../std_types.h" 2
...
# 43 "./ECU_Layer/Chr_LCD/ecu_chr_lcd.h"
typedef struct{
    pin_config_t lcd_rs;
    pin_config_t lcd_en;
...
Std_ReturnType eeprom_read_byte(uint16 address,uint8 *read_data);
# 21 "./application.h" 2
...
# 38 "./MCAL_Layer/Timer0/hal_timer0.h"
typedef enum {
...
typedef enum{
    TIMER0_T0CKI_HIGH_TO_LOW,
    TIMER0_T0CKI_LOW_TO_HIGH
}timer0_t0cki_transition_t;

typedef enum{
    TIMER0_PRESCALER_ENABLE,
    TIMER0_PRESCALER_DISBALE
}timer0_prescaler_state_t;
...
typedef struct{
 ...
 timer0_size_t size;
 timer0_mode_t mode;
 timer0_t0cki_transition_t transition_state;
 timer0_prescaler_state_t prescaler_state;
 timer0_prescaler_select_t prescaler_select;
 uint16 timer0_initial_value;
}timer0_t;

Std_ReturnType timer0_initialize(const timer0_t *timer0);
Std_ReturnType timer0_read_value(const timer0_t *timer0,uint16 *timer0_value);
Std_ReturnType timer0_Write_Value(const timer0_t *timer0,uint16 *timer0_value);
# 23 "./application.h" 2

# 1 "./MCAL_Layer/Timer1/hal_timer1.h" 1
# 38 "./MCAL_Layer/Timer1/hal_timer1.h"
typedef enum{
 TIMER1_READ_WRITE_ONE_16BIT_OPERATION,
 TIMER1_READ_WRITE_TWO_8BIT_OPERATION
}timer1_read_write_mode_enable_t;
...
}timer1_oscillator_enable_or_disable_state_t;

typedef enum{
 TIMER1_EXTERNAL_CLOCKS_T1CKI_SYNCHRONIZE,
 TIMER1_EXTERNAL_CLOCKS_T1CKI_ASYNCHRONIZE
}timer1_external_clocks_t1cki_synch_or_asynch_t;


typedef enum{
 TIMER1_TIMER_MODE,
 TIMER1_COUNTER_MODE_RISING_EDGE
}timer1_mode_t;
...
typedef struct {
   ...
   timer1_mode_t mode;
   timer1_oscillator_enable_or_disable_state_t osc_state;
   timer1_prescaler_select_t prescaler;
   timer1_external_clocks_t1cki_synch_or_asynch_t t1cki_state;
   timer1_read_write_mode_enable_t read_write_state;
   uint16 timer1_initial_value;
}timer1_t;

Std_ReturnType timer1_initialize(const timer1_t *timer1);
Std_ReturnType timer1_Read_Value(const timer1_t *timer1,uint16 *timer1_value);
Std_ReturnType timer1_Write_Value(const timer1_t *timer1,uint16 *timer1_value);
# 24 "./application.h" 2

# 1 "./MCAL_Layer/Timer2/hal_timer2.h" 1
# 49 "./MCAL_Layer/Timer2/hal_timer2.h"
typedef struct{
...
# 66 "./MCAL_Layer/usart/hal_usart.h"
typedef enum{
...
# 32 "./MCAL_Layer/SPI/hal_spi.h"
typedef enum {
    SPI_MASTER_MODE_CLOCK_DIV_BY_4,
    SPI_MASTER_MODE_CLOCK_DIV_BY_16,
    SPI_MASTER_MODE_CLOCK_DIV_BY_64,
    SPI_MASTER_MODE_CLOCK_TIMER2_DIV_BY_2,
    SPI_SLAVE_MODE_WITH_SS_ENABLED,
    SPI_SLAVE_MODE_WITH_SS_DISABLED
}spi_mode_select_t;

typedef enum{
    SPI_TRANSMIT_OCCURE_FROM_ACTIVE_TO_IDLE_CLOCK,
    SPI_TRANSMIT_OCCURE_FROM_IDLE_TO_ACTIVE_CLOCK
}spi_clock_phase_t;

typedef enum{
    SPI_NNPUT_SAMPLE_DATA_AT_END_OUTPUT_TIME,
    SPI_NNPUT_SAMPLE_DATA_AT_MIDDLE_OUTPUT_TIME
}spi_sample_data_t;

typedef enum{
    SPI_CLOCK_POLARITY_IDLE_STATE_IS_HIGH,
    SPI_CLOCK_POLARITY_IDLE_STATE_IS_LOW
}spi_clock_polarity_t;

typedef enum{
    SPI_MASTER_SELECTED,
    SPI_SLAVE_SELECTED
}spi_master_or_slave_selected_t;

typedef struct{
 spi_master_or_slave_selected_t master_or_slave;
 spi_mode_select_t spi_mode;
 spi_clock_phase_t clock_phase;
 spi_clock_polarity_t clock_polarity;
 spi_sample_data_t sample_data_state;
 pin_config_t master_selcet_pin;
}spi_t;

Std_ReturnType spi_master_initialize(const spi_t *spi);
Std_ReturnType spi_slave_initialize(const spi_t *spi);
Std_ReturnType spi_write_data(uint8 data);
Std_ReturnType spi_read_data(uint8 *data);
# 28 "./application.h" 2

# 1 "./MCAL_Layer/I2C/hal_i2c.h" 1
# 57 "./MCAL_Layer/I2C/hal_i2c.h"
typedef struct{
    uint32 i2c_clock_frequancy_for_master;
    uint8 i2c_slave_address_for_slave;
    uint8 ic2_mode_selected;
    uint8 i2c_master_mode_transmit_or_receiver;
}i2c_t;

Std_ReturnType i2c_master_mode_intialize(const i2c_t *i2c);
Std_ReturnType i2c_slave_mode_intialize(const i2c_t *i2c);
Std_ReturnType i2c_send_start_condition(const i2c_t *i2c);
Std_ReturnType i2c_send_stop_condition(const i2c_t *i2c);
Std_ReturnType i2c_send_repeated_start_condition(const i2c_t *i2c);

Std_ReturnType i2c_send_address(uint8 Address_With_WR_Bit,uint8 *address_match_status);
Std_ReturnType i2c_send_data(uint8 data);
Std_ReturnType i2c_receive_data(uint8 *data);
# 29 "./application.h" 2
