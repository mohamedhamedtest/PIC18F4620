...
# 10 "ECU_Layer/KeyPad/ecu_keypad.h"
...
# 12 "ECU_Layer/KeyPad/../../MCAL_Layer/GPIO/../std_types.h"
...
# 12 "ECU_Layer/KeyPad/../../MCAL_Layer/GPIO/../std_types.h" 2
...
# 13 "ECU_Layer/KeyPad/../../MCAL_Layer/GPIO/../std_types.h" 2
...
# 10 "ECU_Layer/KeyPad/ecu_keypad.h" 2
...
Std_ReturnType keypad_initialize(const keypad_t *keypad)
{
    ...
    if(((void*)0) == keypad){
        ...
        for(int i=0;i<4;i++)
        {
            ret=gpio_pin_intialize(&(keypad->keypad_row_pins[i]));
        }
         for(int i=0;i<4;i++)
        {
            ret=gpio_pin_direction_intialize(&(keypad->keypad_columns_pins[i]));
        }
    }
    return ret;
}
...
{
          Std_ReturnType ret = (Std_ReturnType)0x01;
          logic_t status_pin=GPIO_LOW;

          *status=(0xff);

    if((((void*)0) == keypad)||(((void*)0) == status)){
        ret = (Std_ReturnType)0x00;
    }
    else{
        for(int row=0; row<4; row++)
        {
            for(int i=0; i<4; i++)
            {
                ret=gpio_pin_write_logic(&(keypad->keypad_row_pins[i]),GPIO_LOW);
            }
            ret=gpio_pin_write_logic(&(keypad->keypad_row_pins[row]),GPIO_HIGH);
